...
class InvokeAIAppConfig(BaseSettings):
    ...
    Attributes:
        ...
        device: Preferred execution device. `auto` will choose the device depending on the hardware platform and the installed torch capabilities.<br>Valid values: `auto`, `cpu`, `cuda`, `xpu`, `mps`, `cuda:N` (where N is a device number), `xpu:N` (where N is a device number)
    ...
    # DEVICE
    device:                      str = Field(default="auto",                description="Preferred execution device. `auto` will choose the device depending on the hardware platform and the installed torch capabilities.<br>Valid values: `auto`, `cpu`, `cuda`, `xpu`, `mps`, `cuda:N` (where N is a device number), `xpu:N` (where N is a device number)", pattern=r"^(auto|cpu|mps|cuda(:\d+)?|xpu(:\d+)?)$")
